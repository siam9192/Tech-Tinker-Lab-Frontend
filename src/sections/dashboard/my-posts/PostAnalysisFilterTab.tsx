 ...
 function PostAnalysisFilterTab({onChange}:ITab) {
  ...
  return (
    <section className='p-5 flex items-center gap-2'>
        {
          tabs.map((tab,i)=>(
            <button onClick={()=>handelSelectTab(i)} className={`text-xl font-bold ${activeTab === i ?'text-primary-color':'text-black dark:text-white' } hover:scale-110 duration-100 ease-in-out`}>{tab.title}</button>
          ))
        }
    </section>
  )
