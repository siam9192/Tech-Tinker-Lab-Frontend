...
function ProfileEditForm({ successFn }: IProfileEditForm) {
  const coverPhotoInputRef = useRef<HTMLInputElement>(null);
  const profilePhotoInputRef = useRef<HTMLInputElement>(null);
  const [changedCoverPhoto, setChangedCoverPhoto] = useState<File | null>(null);
  const [changedProfilePhoto, setChangedProfilePhoto] = useState<File | null>(
    null,
  );
  ...
  const handelImageInputOnChange = (
    name: 'profile' | 'cover',
  ...
  return (
    <section>
      ...
      <input
        ...
        onChange={(e) => handelImageInputOnChange('cover', e)}
        type="file"
        className="hidden"
      />
      ...
      <input
        ...
        onChange={(e) => handelImageInputOnChange('profile', e)}
        type="file"
        className="hidden"
      />

      <div
        onClick={() => openInput(coverPhotoInputRef.current)}
        className="relative"
      >
        <img
          src={coverPhoto}
          alt=""
          className="h-60 rounded-lg w-full relative "
        />
      ...
      <div
        onClick={() => openInput(profilePhotoInputRef.current)}
        className="bg-black  dark:bg-white  z-40 p-1 -mt-14 rounded-full  mx-auto relative w-fit "
      >
        <img
          src={profilePhoto}
          className="-z-10  size-20 md:size-40 rounded-full"
          alt=""
        />
      ...
      <Form
        ...
        resolver={zodResolver(ZodValidations.editProfileValidation)}
      >
