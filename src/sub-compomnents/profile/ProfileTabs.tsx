  ...
  return (
    <div className='mt-5 bg-white dark:bg-dark-light p-2 md:p-5 min-h-[60vh]'>
        <div className='flex items-center gap-5 md:gap-10  overflow-x-auto md:overflow-hidden py-2'>
            {
                tabs.map((tab,index)=>(
                    <button onClick={()=>setActiveTab(index)} className={`text-xl md:text-2xl font-medium ${activeTab ===index ?'text-primary-color':'' }`}>
                        {tab.title}
                    </button>
                ))
            }
        ...
        <div className='mt-5'>
           {
            ...
            :
            activeTab === 1 ?<ProfileFollowing username={username}/>
            :
            <></>
           }
